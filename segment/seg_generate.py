...
class Gen(Sequence):
    ...
    def _augments(self, x, fun = None):
        ...
        if  self.worker: 
            ...
            result = [pool.apply_async(self._get_data, args = (im, fun, self.state)) for im in x]
            pool.close()
            pool.join()
            if self.state == 'train':
                for re in result:
            ...
            for im in x:
                imx, imgx = self._get_data(im, fun, self.state)
                collection_x.append(imx)
                collection_gx.append(imgx)             
            return  np.asarray(collection_x), np.asarray(collection_gx)
    ...
    def _get_data(self, im, preproce_fun, state):
        if self.dataset != 'mnist':
            im = cv2.imread(im)

        if isinstance(self.target_size, (tuple, list)) and len(self.target_size) == 2:
